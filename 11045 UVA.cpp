...
int fordFulkerson(int source,int sink)
{
    ...
    while(isPath(source,sink))
    {
        ...
        {
            u = parent[v];
            min_res_cap = min(min_res_cap,g[u][v]);
            v = parent[v];
        }
        v = sink;
        while(parent[v]!=-1)
...
int main()
{
    ...
    while(test--)
    {
        ...
        cin>>n>>m;
        mem(g,0);
        int super_sink = 7+m*3;
        string s1,s2;
        for(int i=1;i<=6;i++)   g[0][i] = n/6;
        for(int i=1;i<=m;i++)
        {
            cin>>s1>>s2;
            ...
            int x = shirt_id[s1];
            int y = shirt_id[s2];
            g[x][u] = g[u][7+m*2+i-1] = 1;
            g[y][v] = g[v][7+m*2+i-1] = 1;
            g[7+m*2+i-1][super_sink] = 1;
        }
        int ans = fordFulkerson(0,super_sink);
        if(ans<m)
            cout<<"NO"<<endl;
        else
            cout<<"YES"<<endl;
